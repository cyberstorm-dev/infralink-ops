...
def test_too_many_owned_jumps_fail_before_mutation(monkeypatch: pytest.MonkeyPatch) -> None:
    import infralink_ops.egress_snat as module

    calls: list[list[str]] = []
    owned_jump = "-A POSTROUTING -j INFRALINK_EGRESS_SNAT"
    ...
    def run(argv: list[str], **_kwargs: object) -> object:
        ...
        if argv == ["/usr/sbin/iptables", "-t", "nat", "-S", "INFRALINK_EGRESS_SNAT"]:
            return type("Result", (), {"returncode": 0, "stdout": "-N INFRALINK_EGRESS_SNAT\n"})()
        if argv == ["/usr/sbin/iptables", "-t", "nat", "-S", "POSTROUTING"]:
            return type("Result", (), {"returncode": 0, "stdout": (owned_jump + "\n") * 129})()
        pytest.fail("SNAT reconciliation must not mutate an oversized owned-jump state")
